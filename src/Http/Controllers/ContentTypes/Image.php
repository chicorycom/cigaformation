...
class Image extends BaseType
{
    public function handle()
    {
        if ($this->request->hasFile($this->row->field)) {
            ...
            if ($this->is_animated_gif($file)) {
                Storage::disk(config('filesystems.disks'))->put($fullPath, file_get_contents($file), 'public');
                $fullPathStatic = $path.$filename.'-static.'.$file->getClientOriginalExtension();
                Storage::disk(config('chicorycom.storage.disk'))->put($fullPathStatic, (string) $image, 'public');
            } else {

                Storage::disk(config('chicorycom.storage.disk'))->put($fullPath, (string) $image, 'public');
            ...
            if (isset($this->options->thumbnails)) {
                foreach ($this->options->thumbnails as $thumbnails) {
                    ...
                    Storage::disk(config('chicorycom.storage.disk'))->put(
    ...
    protected function generateFileName($file, $path)
    {
        if (isset($this->options->preserveFileUploadName) && $this->options->preserveFileUploadName) {
            ...
            while (Storage::disk(config('chicorycom.storage.disk'))->exists($path.$filename.'.'.$file->getClientOriginalExtension())) {
